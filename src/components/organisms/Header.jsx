...
const Navbar = () => {
  ...
  return (
    <header className='flex justify-between items-center md:py-4 md:px-10 p-4 bg-page-header text-light-primary text-xs md:text-lg lg:text-xl'>
        <div name="header-container" className='flex items-center'>
            <div name="header-nav" className='flex items-center gap-4 md:gap-14 lg:gap-18'>
                <Link to="/Home" className='hidden md:flex'><img className='h-7 lg:h-9' src={webLogo} alt="Logo Chill"/></Link>
        ...
        <div name="header-profile" className='flex items-center'>
            <div name="profile-dropdown" className='flex items-center gap-2 md:gap-4'>
                <img className='w-5 md:w-10 lg:w-12' src={ProfileIcon} alt="Profile Icon"/>
                <p className='font-medium'>R Aditya</p>
                <div className='group'>
                    <FontAwesomeIcon icon="fa-solid fa-angle-down"/>
                    <section className='invisible group-hover:visible delay-200 absolute w-35 md:w-55 lg:w-65 top-10 md:top-15 lg:top-17 right-4 md:right-10 bg-page-header rounded-sm p-4 md:p-6 lg:p-8 outline-1 outline-light-secondary/10 z-11'>
