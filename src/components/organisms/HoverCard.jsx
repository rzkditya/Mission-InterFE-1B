...
const HoverCard = ({movie: {category, watched, duration = '2j 33m', episodes = '16 Episodes', age_rating = '13+', genres = [], poster_path}, onShowDetail}) => {
    const isFilm = category === 'Films'
    const meta = isFilm ? <p>{duration}</p> : <p>{episodes}</p>
            ...
            <div className={baseStyle}>
                ...
                <section className='flex flex-col px-4 pb-4 gap-2 text-sm lg:text-xl text-light-primary'>
                    <div className='flex justify-between items-center'>
                        ...
                        <Button variant='clean-white' className='w-8 lg:w-12 h-8 lg:h-12 rounded-full'><FontAwesomeIcon icon="fa-solid fa-angle-down" /></Button>
                    </div>
                    {seriesWatched}
    ...
    return (  
        <div className={baseStyle}>
            ...
            <section className='flex flex-col px-4 pb-4 gap-4 text-sm lg:text-xl text-light-primary'>
                <div className='flex justify-between items-center'>
                    <div className='flex gap-4 items-center'>
                        <Button variant='' className='w-6 lg:w-10 h-6 lg:h-10 bg-light-primary rounded-full'><FontAwesomeIcon icon="fa-solid fa-play" style={{color:"#000000",}}/></Button>
                        <Button variant='clean-white' className='w-8 lg:w-12 h-8 lg:h-12 rounded-full'><FontAwesomeIcon icon="fa-solid fa-check" /></Button>
                ...
                <div className='flex items-center gap-2'>
                    <div className='w-10 lg:w-12 rounded-full bg-background/30 text-center text-light-secondary'>{age_rating}</div>
                    {meta}
                </div>
                <div>
                    <ul className='flex justify-evenly items-center text-xs lg:text-xl text-center text-light-secondary gap-2'>
